...
type ProjectPreviewProps = {
  ...
  deadline: string
  problem: string
  audience: "Clientes" | "Projeto interno" | "Pessoal"
  platforms: string[]
  actions: string
...
export default function ProjectPreview({ item, title, category, deadline, problem, audience, platforms, actions, programming_language, minBudget, maxBudget }: ProjectPreviewProps) {
  const router = useRouter();
  const openModal = () => {
    router.push(`?modal=project&id=${encodeURIComponent(item)}`, {
      scroll: false,
    });
  };
